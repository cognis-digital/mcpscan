...
**mcpscan** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `mcpscan` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
mcpscan ... --format json | cognis-connect emit --to sigma   # Sigma rules
mcpscan ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
mcpscan ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
mcpscan ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `mcpscan` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="mcpscan") for rec in records]   # records = your JSON output
