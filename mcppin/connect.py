"""Native cognis-connect emit for mcppin — forward findings to any platform.

Maps mcppin's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    mcppin ... --format json | mcppin-emit --to stix
    mcppin-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['severity'] = 'medium'
        out['type'] = 'vulnerability'
        out['description'] = rec.get('inputSchema', {}).get('description', '')
