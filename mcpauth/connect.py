"""Native cognis-connect emit for mcpauth — forward findings to any platform.

Maps mcpauth's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    mcpauth ... --format json | mcpauth-emit --to stix
    mcpauth-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out.pop('user', None)
        out.pop('action', None)
        out['title'] = rec.get('event', 'Unknown Event')
        out['severity'] = rec.get('severity', 'info')
        out['type'] = rec.get('type', 'auth')
        out['description'] = rec.get('message', 'No description available')
