...
(Vendored into mcpscan as ``mcpscan.ai_backend`` — imported via
``from . import ai_backend``. The module is stdlib-only and carries no relative
imports, so the canonical ``tools/_shared/cognis_ai_backend.py`` drops in
unchanged as a package module.)
